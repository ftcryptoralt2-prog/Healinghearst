...
var MemStorage = class {
  ...
  initializeDefaultData() {
    ...
    defaultQuotes.forEach((quote) => {
      ...
      this.quotes.set(id, {
        ...
        author: quote.author,
        approved: true,
        // Default quotes are pre-approved
  ...
  async getApprovedQuotes() {
    return Array.from(this.quotes.values()).filter((quote) => quote.approved).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  }
  async getAllQuotes() {
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/quotes", async (req, res) => {
    try {
      const quotes2 = await storage.getApprovedQuotes();
      res.json(quotes2);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch quotes" });
    }
  });
  app2.get("/api/quotes/pending", requireAdmin, async (req, res) => {
    try {
      const quotes2 = await storage.getPendingQuotes();
      res.json(quotes2);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch pending quotes" });
    }
  });
  app2.post("/api/quotes/:id/approve", requireAdmin, async (req, res) => {
    try {
  ...
  app2.post("/api/videos", requireAdmin, async (req, res) => {
    try {
